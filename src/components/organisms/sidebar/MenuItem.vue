<template>
  ...
  >
    ...
    <div
      ...
      ref="topContainerRef"
      :style="{
        [MakeSpace ? 'bottom' : 'top']:
          `calc(${ContainerOffsetYConputed} - 1px)`,
...
export default {
  ...
  watch: {
    routerPushBlockList(value) {
      // Exit early if conditions aren't met
      if (!(this.keepOneMenuOpenAtAtime || this.closeOpenMenuOnHrefPush))
        return;

      // Exit if the current item matches the value
      if (this.item[this.symbolId] === value) return;

      if (this.item.children) {
        let isFound = false;

        // Recursive function to backtrack through children
        const backTrack = arr => {
          if (isFound) return;

          for (let i = 0; i < arr.length; i++) {
            if (arr[i][this.symbolId] === value) {
              isFound = true;
              break;
            }
            if (arr[i].children) {
        ...
        // Start backtracking through the item's children
        backTrack(this.item.children);

        // If not found, close the item's children
        if (!isFound) {
  ...
  methods: {
    ...
    clickCompose() {
      ...
      if (
        (this.vueRouterEnabel && this.item?.href && this.$router) ||
        this.keepOneMenuOpenAtAtime
      ) {
    ...
    setItemOffsetHeight() {
      if (this.depth == 0) {
        ...
        const x1 =
          this.$refs['topContainerRef']?.getBoundingClientRect().height;
        let z = 0;
        if (this.item?.children) {
