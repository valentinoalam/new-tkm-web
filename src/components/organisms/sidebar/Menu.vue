...
import { initAwsomeSideBar } from '@/composables/useAwsSideBar';
import {
  useClickOutSide,
  useAutoCollapse,
} from '@/composables/useAwsClickOutside';
import { initAwsomeRouter } from '@/composables/useRouter';
  ...
  setup(props, context) {
    ...
    watch(
      ...
      currentCollapsed => {
        ...
        // Toggle overlay if `overLayerOnOpen` is enabled
        if (props.overLayerOnOpen) {
          overLayer.value = !currentCollapsed;
        }

        // Handle sidebar listener if `closeOnClickOutSide` is enabled
        if (props.closeOnClickOutSide) {
          if (currentCollapsed) removeSideBarListner();
          else addSideBarListner();
        }
      }
    );
    watch(
      ...
      val => {
        // Re-add or remove sidebar listener based on the new value
        if (val) addSideBarListner();
        else removeSideBarListner();
      }
    );
    watch(
      () => props.miniMenu,
...
<style lang="scss">
@use '@/assets/scss/vue-awesome-sidebar.scss';
</style>
