<template>
  ...
  <div class="flex bg-gray-50 font-lexend dark:bg-gray-900">
    <div v-if="!$route.meta.hideNav" class="lg:block">
      ...
      >
        <perfect-scrollbar class="h-screen">
          <Sidebar v-if="!$route.meta.hideNav" @sidebarToggle="isOpen" />
          <!-- <sidebarlist
    ...
    >
      <admin-header :title="routeName" v-if="!route.meta.hideNav" />
      <main class="px-4 bg-gray-200">
...
onMounted(() => {
  const bodyScrollElement = document.querySelector(
    '#body-scroll'
  ) as HTMLElement | null;
